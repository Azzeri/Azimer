...
<template>
    <AppLayout title="Users">
        <Table striped :resource="users" class="mt-10">
            <template v-slot:tableFilter="slotProps">
                <button
                    @click="openModal('create-user-modal')"
                    class="btn btn-sm"
                >
                    Dodaj
                </button>
            ...
            <template #cell(fire_brigade_unit_id)="{ item: user }">
                {{ user.fire_brigade_unit ? user.fire_brigade_unit.name : "-" }}
            </template>
            <template #cell(phone)="{ item: user }">
                {{ user.phone || "-" }}
            </template>
        </Table>
